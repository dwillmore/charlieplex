...
// This is an example for an 8 signal charlieplexed display using only port C
#define CHARLIE_PINS_N 8
...
#define CHARLIE_CFGLRC_MASK 	(~(GPIO_CFGLR_MODE0 | \
				...
				GPIO_CFGLR_CNF6 | \
				GPIO_CFGLR_CNF7))
// We use none of port D
#define CHARLIE_CFGLRD_MASK	0xffffffff
...
// by virtual pin number.  Virtual pin 0 is PC0 though pin 7 being PC7
const struct {
	uint32_t cfglrc;
	uint32_t cfglrd;
	uint16_t bshrc;
